...
export function trim(arg: string): string {
    return arg.replace(/^(["'])|(['"])$/g, "").replace(/\.ts$/, "");
}
 ...
 * @param result
 */
export function reverseMap<T>(result: Map<T, number>): Map<number, Set<T>> {
    const reversResult = new Map<number, Set<T>>();
    for (const [node, sccIndex] of result) {
...
// set of colors that are distinct for human eyes
const colors = [
...
export function getAColor(index: number): string {
